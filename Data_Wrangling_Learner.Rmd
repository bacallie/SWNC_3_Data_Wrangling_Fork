...
groundhog_subset <- groundhog_dat %>%
  dplyr::filter(record == "No Shadow" 
  & feb_avg_temp_penn > 32)
  
dplyr::mutate(groundhog_subset
...
groundhog_edited <- groundhog_edited %>%
  # Call rename_with. Use the function (copied from online) to substitute Feb_ for the longer string.
  rename_with(function(x){gsub("March.Average.Temperature..","march_",x)})
...
# putting in NA values where there are empty values
groundhog_raw[groundhog_raw == ""] <- NA

# cleaning the data: renaming columns and removing NAs
groundhog_edited <- groundhog_raw %>%
    rename_with(function(x){gsub("March.Average.Temperature..","march_",x)}) %>%
    rename_with(function(x){gsub("February.Average.Temperature..","feb_",x)}) %>%
    clean_names() %>%
...
# filtering the data to only show when phil didn't see his shadow and penn was > 32 in feb
groundhog_filtered <- groundhog_edited %>%
  filter(punxsutawney_phil == "No Shadow") %>%
  filter(feb_pennsylvania > 32)

# converting the year column to date class
groundhog_filtered <- groundhog_filtered %>%
  transform(year = ISOdate(year, 1, 1))

# filtering the year column for dates between 1900 and 2001
  groundhog_filtered <- groundhog_filtered %>%
...
# changing it so the year column is in front of the phil column
  groundhog_layout <- groundhog_layout %>% 
  # columns will move to the front in the order written
  relocate(punxsutawney_phil, .after =year) %>%
    relocate(feb_pennsylvania, .after = punxsutawney_phil)
